    ...
    public void typeEntry(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.num9_button:
                ...
                if (cursor == 0)
                    insertEntry(((Button)view).getText().toString());
                else {
                    if (isEligible(Element.Numbers))
                        insertEntry(((Button)view).getText().toString());
                }
                break;
            ...
            case R.id.pow_button:
                ...
                // TODO: ignore if ineligible, insert number otherwise.
                if (cursor > 0) {
                    if (isEligible(Element.Operators))
                        insertEntry(((Button) view).getText().toString());
                }
                break;
            ...
            case R.id.eval_button:
                if (CalculatorParser.hasCorrectFormat(exp.toString())) {
                    try {
                        ...
                        Toast.makeText(this,"Invalid computation. Please check the expression.",Toast.LENGTH_SHORT);
                    }
                }
                break;
